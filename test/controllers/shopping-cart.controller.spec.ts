...
describe('Shopping cart Controller', () => {
  ...
  it('should get all cart items', async () => {
    ...
    expect(response.body).toEqual(
      expect.arrayContaining([{ ...objCart, userId: user.id }]),
    );
  });
  ...
  it('should add item to cart', async () => {
    ...
    expect(
      response.body.find((item: { partId: number }) => item.partId === 3),
    ).toEqual(
      expect.objectContaining({ ...objCart, userId: user.id, partId: 3 }),
    );
  });
  ...
  it('should delete cart item', async () => {
    ...
    expect(response.body.find((item: { partId: number }) => item.partId === 1))
      .toBeUndefined;
  });
